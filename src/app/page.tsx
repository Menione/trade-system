...
const STEPS = [
  {id:1,label:"Invoice蜈･蜉・,icon:"搭"},
  {id:2,label:"Packing List",icon:"逃"},
  {id:3,label:"蜀・ｮｹ遒ｺ隱・,icon:"笨・},
  {id:4,label:"PDF逕滓・",icon:"塘"},
  {id:5,label:"繝｡繝ｼ繝ｫ騾∽ｻ・,icon:"透"},
  {id:6,label:"蜃ｺ闕ｷ螳御ｺ・,icon:"圓"},
];
...
function runValidation(invoice: any, packingItems: any[]) {
  ...
  if (items.length === 0) {
    errors.push({field:"hsCode",msg:"蜩∫岼縺梧悴逋ｻ骭ｲ縺ｧ縺吶ょ刀逶ｮ繧定ｿｽ蜉縺励※HS繧ｳ繝ｼ繝峨ｒ蜈･蜉帙＠縺ｦ縺上□縺輔＞縲・,risk:"HIGH",step:1});
  } else if (!items.every((i:any) => i.hsCode && i.hsCode.trim() !== "")) {
    errors.push({field:"hsCode",msg:"HS繧ｳ繝ｼ繝峨′譛ｪ蜈･蜉帙・蜩∫岼縺後≠繧翫∪縺吶る夐未縺ｫ蠢・医〒縺吶・,risk:"HIGH",step:1});
  }
  if (!invoice.incoterms) errors.push({field:"incoterms",msg:"Incoterms縺梧悴驕ｸ謚槭〒縺吶・,risk:"HIGH",step:1});
  if (!invoice.countryOfOrigin) errors.push({field:"countryOfOrigin",msg:"蜴溽肇蝗ｽ縺梧悴蜈･蜉帙〒縺吶・,risk:"HIGH",step:1});
  if (!invoice.currency) errors.push({field:"currency",msg:"騾夊ｲｨ縺梧悴驕ｸ謚槭〒縺吶・,risk:"HIGH",step:1});
  ...
  if (invoiceQty>0 && packingQty>0 && invoiceQty!==packingQty) {
    errors.push({field:"quantity",msg:`謨ｰ驥丈ｸ堺ｸ閾ｴ: Invoice ${invoiceQty}蛟・/ Packing ${packingQty}蛟義,risk:"HIGH",step:2});
  }
  packingItems.forEach((item,idx)=>{
    if (!item.grossWeight||Number(item.grossWeight)===0) {
      errors.push({field:`grossWeight_${idx}`,msg:`繧ｫ繝ｼ繝医Φ${item.cartonNo}縺ｮ邱城㍾驥上′譛ｪ蜈･蜉帙〒縺吶Ａ,risk:"HIGH",step:2});
    }
    if (item.grossWeight&&item.netWeight&&Number(item.grossWeight)<Number(item.netWeight)) {
      errors.push({field:`weight_${idx}`,msg:`繧ｫ繝ｼ繝医Φ${item.cartonNo}縺ｮ邱城㍾驥上′豁｣蜻ｳ驥埼㍼繧医ｊ蟆上＆縺・〒縺吶Ａ,risk:"MEDIUM",step:2});
    }
    if (item.grossWeight&&Number(item.grossWeight)>5000) {
      warnings.push({field:`weight_${idx}`,msg:`繧ｫ繝ｼ繝医Φ${item.cartonNo}縺ｮ驥埼㍼縺・000kg繧定ｶ・∴縺ｦ縺・∪縺吶ら｢ｺ隱阪＠縺ｦ縺上□縺輔＞縲Ａ,risk:"LOW",step:2});
    }
  });
  if (!invoice.shipper) warnings.push({field:"shipper",msg:"Shipper諠・ｱ縺梧悴蜈･蜉帙〒縺吶・,risk:"LOW",step:1});
  if (!invoice.consignee) warnings.push({field:"consignee",msg:"Consignee諠・ｱ縺梧悴蜈･蜉帙〒縺吶・,risk:"LOW",step:1});
  if (items.length>0) {
    const currencies = new Set(items.map((i:any)=>i.currency).filter(Boolean));
    if (currencies.size>1) errors.push({field:"currency",msg:`隍・焚縺ｮ騾夊ｲｨ縺梧ｷｷ蝨ｨ縺励※縺・∪縺・ ${[...currencies].join(", ")}`,risk:"MEDIUM",step:1});
  }
...
function ValidationPanel({invoice,packingItems,onGoToStep}:any) {
  ...
  const icon=riskLevel==="HIGH"?"圷":riskLevel==="MEDIUM"?"笞・・:riskLevel==="LOW"?"兆":"笨・;
  const titleText=riskLevel==="HIGH"?"騾夐未繝ｪ繧ｹ繧ｯ: 驥榊､ｧ縺ｪ繧ｨ繝ｩ繝ｼ縺後≠繧翫∪縺・:riskLevel==="MEDIUM"?"遒ｺ隱阪′蠢・ｦ√↑鬆・岼縺後≠繧翫∪縺・:riskLevel==="LOW"?"霆ｽ蠕ｮ縺ｪ隴ｦ蜻翫′縺ゅｊ縺ｾ縺・:"縺吶∋縺ｦ縺ｮ蠢・磯・岼縺悟・蜉帙＆繧後※縺・∪縺・;
  return (
    <div className={`validation-panel ${riskLevel}`}>
      <div className="validation-title">
        <span>{icon}</span><span>{titleText}</span>
        {riskLevel!=="CLEAR"&&<span className={`risk-badge ${riskLevel}`}>{errors.length}繧ｨ繝ｩ繝ｼ / {warnings.length}隴ｦ蜻・/span>}
      </div>
      {errors.map((e:any,i:number)=>(
        <div key={i} className={`validation-item ${onGoToStep?"validation-item-clickable":""}`}
          ...
          <span className="risk-HIGH">閥 {e.msg}</span>
          {onGoToStep&&e.step&&<span style={{marginLeft:"auto",fontSize:11,color:"var(--blue)",flexShrink:0}}>STEP{e.step}縺ｸ 竊・/span>}
        </div>
      ))}
      {warnings.map((w:any,i:number)=>(
        <div key={i} className={`validation-item ${onGoToStep?"validation-item-clickable":""}`}
          ...
          <span className="risk-LOW">泯 {w.msg}</span>
          {onGoToStep&&w.step&&<span style={{marginLeft:"auto",fontSize:11,color:"var(--blue)",flexShrink:0}}>STEP{w.step}縺ｸ 竊・/span>}
        </div>
      ))}
    </div>
  );
...
function InvoiceForm({invoice,setInvoice,onNext,orgSettings,customers,products}:any) {
  ...
  return (
    <div className="fade-in">
      {invoice.isDraft&&<div className="draft-banner">沈 荳区嶌縺堺ｿ晏ｭ俶ｸ医∩ 窶・譛邨ゆｿ晏ｭ・ {invoice.draftSavedAt||"窶・}</div>}

      {/* 蝓ｺ譛ｬ諠・ｱ */}
      <div className="card">
        <div className="card-header">
          <div><div className="card-title">搭 蝓ｺ譛ｬ諠・ｱ</div></div>
        </div>
        <div className="grid-3" style={{marginBottom:16}}>
          ...
          <div className="field">
            <label className="label"><span className="required-dot">*</span>菴懈・譌･莉・/label>
            <input type="date" className="input" value={invoice.date||""}
              onChange={(e:any)=>setInvoice((v:any)=>({...v,date:e.target.value}))}/>
          </div>
          <div className="field">
            <label className="label">豕ｨ譁・嶌逡ｪ蜿ｷ</label>
        ...
        <div className="grid-3" style={{marginBottom:16}}>
          <div className="field">
            <label className="label"><span className="required-dot">*</span>騾夊ｲｨ</label>
          ...
          <div className="field">
            ...
            <select className="input" value={invoice.status||"draft"}
              ...
              <option value="in_progress">菴懈･ｭ荳ｭ</option>
              <option value="reviewing">遒ｺ隱堺ｸｭ</option>
              <option value="shipped">蜃ｺ闕ｷ貂医∩</option>
              <option value="completed">螳御ｺ・/option>
            </select>
          </div>
        </div>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
              ...
              {customers.length>0&&(
                <select className="input" style={{width:"auto",fontSize:11,padding:"2px 6px"}}
                  ...
                  <option value="">蜿門ｼ募・縺九ｉ驕ｸ謚・/option>
                  {customers.map((c:any)=><option key={c.id} value={c.id}>{c.name}</option>)}
                </select>
              )}
            </div>
            <textarea className="input" value={invoice.consignee||""} rows={3} placeholder={"莨夂､ｾ蜷構n菴乗園\n蝗ｽ"}
              onChange={(e:any)=>setInvoice((v:any)=>({...v,consignee:e.target.value}))}/>
          </div>
        </div>
        <div className="field">
          <label className="label">Notify Party</label>
          <textarea className="input" value={invoice.notifyParty||""} rows={2} placeholder="騾夂衍蜈茨ｼ・/C逋ｺ陦梧凾縺ｪ縺ｩ・・
      ...
      {/* 雋ｿ譏捺擅莉ｶ */}
      <div className="card">
        <div className="card-header"><div className="card-title">圓 雋ｿ譏捺擅莉ｶ</div></div>
      ...
      <div className="card">
        <div className="card-header">
          <div><div className="card-title">逃 蜩∫岼譏守ｴｰ</div><div className="card-subtitle">HS繧ｳ繝ｼ繝峨・蠢・★蜈･蜉帙＠縺ｦ縺上□縺輔＞・磯夐未蠢・茨ｼ・/div></div>
          <div style={{display:"flex",gap:8}}>
            {products.length>0&&(
              <select className="input" style={{width:"auto",fontSize:12,padding:"5px 10px"}}
                ...
                <option value="">陬ｽ蜩√・繧ｹ繧ｿ縺九ｉ霑ｽ蜉</option>
        ...
        {(!invoice.items||invoice.items.length===0)?(
          <div className="empty-state"><div className="empty-icon">逃</div><div className="empty-text">蜩∫岼繧定ｿｽ蜉縺励※縺上□縺輔＞</div></div>
        ):(
          <div style={{overflowX:"auto"}}>
            <table className="items-table">
              <thead>
                <tr>
                  <th style={{width:160}}>陬ｽ蜩∝錐 <span style={{color:"var(--red)"}}>*</span></th>
                  <th style={{width:80}}>謨ｰ驥・/th>
                  <th style={{width:100}}>蜊倅ｾ｡</th>
                  <th style={{width:70}}>騾夊ｲｨ</th>
                  <th style={{width:130}}>HS繧ｳ繝ｼ繝・<span style={{color:"var(--red)"}}>*</span></th>
                  <th style={{width:100,textAlign:"right"}}>蟆剰ｨ・/th>
              ...
              <tbody>
                {invoice.items.map((item:any)=>{
                  ...
                  return (
                    <tr key={item.id}>
                      <td><input className="input" value={item.productName||""} placeholder="陬ｽ蜩∝錐" onChange={(e:any)=>updateItem(item.id,"productName",e.target.value)}/></td>
                      ...
                      <td><button className="btn btn-danger btn-xs" onClick={()=>removeItem(item.id)}>笨・/button></td>
                    </tr>
                  );
                })}
              </tbody>
      ...
      {/* 蛯呵・*/}
      <div className="card">
        <div className="card-header"><div className="card-title">統 蛯呵・・蜃ｺ闕ｷ隧ｳ邏ｰ</div></div>
        <div className="field" style={{marginBottom:12}}>
          <label className="label">蜃ｺ闕ｷ蛯呵・/label>
          <textarea className="input" value={invoice.shippingRemarks||""} rows={2} placeholder="蜃ｺ闕ｷ縺ｫ髢｢縺吶ｋ迚ｹ險倅ｺ矩・ｼ域｢ｱ蛹・欠遉ｺ繝ｻ霈ｸ騾∵擅莉ｶ縺ｪ縺ｩ・・
            onChange={(e:any)=>setInvoice((v:any)=>({...v,shippingRemarks:e.target.value}))}/>
        </div>
        <div className="field">
          <label className="label">騾夐未蛯呵・/label>
          <textarea className="input" value={invoice.remarks||""} rows={2} placeholder="騾夐未荳翫・豕ｨ諢丈ｺ矩・・迚ｹ險倅ｺ矩・
      ...
      <div style={{display:"flex",justifyContent:"space-between",gap:8}}>
        ...
        <button className="btn btn-primary" onClick={onNext}>Packing List蜈･蜉帙∈ 竊・/button>
      </div>
    </div>
  );
}
    ...
    <div className="fade-in">
      {invoiceQty>0&&totalQty>0&&!qtyMatch&&(
        <div className="validation-panel HIGH" style={{marginBottom:16}}>
          <div className="validation-title"><span>圷</span><span>謨ｰ驥丈ｸ堺ｸ閾ｴ繧ｨ繝ｩ繝ｼ</span></div>
          <div className="validation-item">
            <span className="risk-badge HIGH">HIGH</span>
            <span className="risk-HIGH">Invoice蜷郁ｨ・ {invoiceQty}蛟・竕 Packing蜷郁ｨ・ {totalQty}蛟具ｼ亥ｷｮ蛻・ {Math.abs(invoiceQty-totalQty)}蛟具ｼ・/span>
          </div>
        </div>
      )}
      {invoiceQty>0&&totalQty>0&&qtyMatch&&(
        <div className="validation-panel CLEAR" style={{marginBottom:16}}>
          <div className="validation-title"><span>笨・/span><span>謨ｰ驥丈ｸ閾ｴ: {totalQty}蛟・/span></div>
        </div>
      )}

      <div className="card">
        <div className="card-header">
          <div><div className="card-title">逃 譴ｱ蛹・・邏ｰ</div><div className="card-subtitle">豺ｷ霈峨き繝ｼ繝医Φ縺ｯ縲梧ｷｷ霈峨阪・繧ｿ繝ｳ縺ｧ隍・焚蜩∫岼繧定ｿｽ蜉縺ｧ縺阪∪縺・/div></div>
          <div style={{display:"flex",gap:8}}>
            ...
            }}>Invoice 縺九ｉ閾ｪ蜍募渚譏</button>
            <button className="btn btn-primary btn-sm" onClick={addCarton}>+ 繧ｫ繝ｼ繝医Φ霑ｽ蜉</button>
          </div>
        </div>

        {packingItems.length===0?(
          <div className="empty-state"><div className="empty-icon">逃</div><div className="empty-text">縲栗nvoice 縺九ｉ閾ｪ蜍募渚譏縲阪∪縺溘・縲後き繝ｼ繝医Φ霑ｽ蜉縲阪〒髢句ｧ・/div></div>
        ):(
          <div style={{overflowX:"auto"}}>
            <table className="items-table">
              <thead>
                <tr>
                  ...
                  <th style={{width:160}}>陬ｽ蜩∝錐 / 蜀・ｮｹ</th>
                  <th style={{width:65}}>謨ｰ驥・<span style={{color:"var(--red)"}}>*</span></th>
                  <th style={{width:85}}>邱城㍾驥・kg) <span style={{color:"var(--red)"}}>*</span></th>
                  <th style={{width:80}}>豁｣蜻ｳ驥埼㍼(kg)</th>
                  <th style={{width:180}}>蟇ｸ豕・cm) L ﾃ・W ﾃ・H</th>
                  <th style={{width:60}}>豺ｷ霈・/th>
                  <th style={{width:36}}></th>
                </tr>
              </thead>
              <tbody>
                    ...
                    <tr key={p.id}>
                      ...
                      <td>
                        {p.mixed?(
                          <div style={{fontSize:12,color:"var(--text-muted)",padding:"4px 6px"}}>
                            豺ｷ霈峨き繝ｼ繝医Φ・・(p.mixedItems||[]).length}蜩∫岼・・
                          </div>
                        ):(
                          <input className="input" value={p.productName||""} placeholder="陬ｽ蜩∝錐" onChange={(e:any)=>update(p.id,"productName",e.target.value)}/>
                        )}
                      </td>
                      <td>
                        {p.mixed?(
                      ...
                      <td style={{textAlign:"center"}}>
                        <button className={`btn btn-xs ${p.mixed?"btn-amber":"btn-secondary"}`}
                          ...
                          }}>{p.mixed?"豺ｷ霈我ｸｭ":"豺ｷ霈・}</button>
                      </td>
                      <td><button className="btn btn-danger btn-xs" onClick={()=>remove(p.id)}>笨・/button></td>
                    </tr>
                    {p.mixed&&(
                      <tr key={`${p.id}-mixed`}>
                        <td></td>
                          ...
                          <div className="mixed-carton-items">
                            {(p.mixedItems||[]).map((mi:any)=>(
                              <div key={mi.id} className="mixed-carton-item">
                                <span style={{fontSize:11,color:"var(--text-muted)",minWidth:16}}>笏・/span>
                                <input className="input" style={{flex:2}} value={mi.productName||""} placeholder="陬ｽ蜩∝錐" onChange={(e:any)=>updateMixedItem(p.id,mi.id,"productName",e.target.value)}/>
                                <input className="input" style={{width:70}} type="number" value={mi.quantity||""} placeholder="謨ｰ驥・ onChange={(e:any)=>updateMixedItem(p.id,mi.id,"quantity",e.target.value)}/>
                                <span style={{fontSize:11,color:"var(--text-muted)"}}>蛟・/span>
                                <button className="btn btn-danger btn-xs" onClick={()=>removeMixedItem(p.id,mi.id)}>笨・/button>
                              </div>
                            ))}
                            <button className="btn btn-secondary btn-xs" style={{marginLeft:20,marginTop:2}} onClick={()=>addMixedItem(p.id)}>+ 蜩∫岼霑ｽ蜉</button>
                          </div>
                        </td>
                        <td></td>
                      </tr>
        ...
        {packingItems.length>0&&(
          <div style={{display:"flex",gap:24,marginTop:12,padding:"12px 16px",background:"#F7F7F5",borderRadius:"var(--radius-lg)"}}>
            <div><div className="total-label">邱上き繝ｼ繝医Φ謨ｰ</div><div className="total-value">{packingItems.length} ctns</div></div>
            <div><div className="total-label">蜷郁ｨ域焚驥・/div><div className="total-value" style={{color:!qtyMatch&&invoiceQty>0?"var(--red)":undefined}}>{totalQty} pcs</div></div>
...
function ReviewPage({invoice,packingItems,onNext,onBack,setStep}:any) {
  ...
  const checks=[
    ...
    {label:"蜩∫岼縺・莉ｶ莉･荳翫≠繧・,ok:(invoice.items?.length||0)>0},
    {label:"蜈ｨ蜩∫岼縺ｫHS繧ｳ繝ｼ繝牙・蜉帶ｸ医∩",ok:(invoice.items||[]).length>0&&(invoice.items||[]).every((i:any)=>i.hsCode&&i.hsCode.trim()!=="")},
    {label:"Incoterms 驕ｸ謚樊ｸ医∩",ok:!!invoice.incoterms},
    {label:"蜴溽肇蝗ｽ 蜈･蜉帶ｸ医∩",ok:!!invoice.countryOfOrigin},
    {label:"Packing List 菴懈・貂医∩",ok:packingItems.length>0},
    {label:"Invoice/Packing 謨ｰ驥丈ｸ閾ｴ",ok:(()=>{const iq=(invoice.items||[]).reduce((s:number,i:any)=>s+Number(i.quantity||0),0);const pq=packingItems.reduce((s:number,p:any)=>s+Number(p.quantity||0),0);return iq===pq&&iq>0})()},
    {label:"驥埼㍼蜈･蜉帶ｸ医∩",ok:packingItems.every((p:any)=>p.grossWeight&&Number(p.grossWeight)>0)},
  ];
  ...
  return (
    <div className="fade-in">
      ...
      <div className="grid-2" style={{marginBottom:16}}>
        <div className="card">
          <div className="card-title" style={{marginBottom:14}}>搭 繝√ぉ繝・け繝ｪ繧ｹ繝・({score}/{checks.length})</div>
        ...
        <div className="card">
          <div className="card-title" style={{marginBottom:14}}>投 Invoice 繧ｵ繝槭Μ繝ｼ</div>
          <table style={{width:"100%",fontSize:13}}>
            <tbody>
              {[
                ["Invoice No",invoice.invoiceNo||"窶・],
                ["豕ｨ譁・嶌逡ｪ蜿ｷ",invoice.orderNo||"窶・],
                ["菴懈・譌･莉・,invoice.date||"窶・],
                ["謾ｯ謇墓悄髯・,invoice.paymentDue||"窶・],
                ["Incoterms",invoice.incoterms||"窶・],
                ["蜴溽肇蝗ｽ",invoice.countryOfOrigin||"窶・],
                ["霈ｸ騾∵婿豕・,invoice.shippingMethod||"窶・],
                ["蜩∫岼謨ｰ",`${invoice.items?.length||0}莉ｶ`],
                ["蜷郁ｨ磯≡鬘・,`${currency} ${formatAmount(total,currency)}`],
                ["邱上き繝ｼ繝医Φ謨ｰ",`${packingItems.length} ctns`],
                ["邱城㍾驥・,`${packingItems.reduce((s:number,p:any)=>s+Number(p.grossWeight||0),0).toFixed(2)} kg`],
              ].map(([k,v]:any)=>(
                <tr key={k}>
...
function OutputPage({invoice,packingItems,onBack,orgSettings}:any) {
  ...
  return (
    <div className="fade-in">
      <div className="tabs">
        <button className={`tab ${activeDoc==="invoice"?"active":""}`} onClick={()=>setActiveDoc("invoice")}>搭 Invoice</button>
        <button className={`tab ${activeDoc==="packing"?"active":""}`} onClick={()=>setActiveDoc("packing")}>逃 Packing List</button>
      </div>
      <div className="card">
        <div className="card-header">
          <div className="card-title">{activeDoc==="invoice"?"Invoice 繝励Ξ繝薙Η繝ｼ":"Packing List 繝励Ξ繝薙Η繝ｼ"}</div>
          <div style={{display:"flex",gap:8}}>
            <button className="btn btn-secondary btn-sm" onClick={handlePrint}>蜜・・PDF蜊ｰ蛻ｷ</button>
          </div>
        </div>
          ...
          {activeDoc==="invoice"?(
            <>
              ...
              <div className="meta-grid">
                <div className="meta-item"><div className="meta-key">Invoice No.</div><strong>{invoice.invoiceNo||"窶・}</strong></div>
                <div className="meta-item"><div className="meta-key">Date</div>{invoice.date||"窶・}</div>
                <div className="meta-item"><div className="meta-key">Order No.</div>{invoice.orderNo||"窶・}</div>
                <div className="meta-item"><div className="meta-key">Payment Due</div>{invoice.paymentDue||"窶・}</div>
                <div className="meta-item"><div className="meta-key">Incoterms</div>{invoice.incoterms||"窶・}</div>
                <div className="meta-item"><div className="meta-key">Country of Origin</div>{invoice.countryOfOrigin||"窶・}</div>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:12}}>
                <div><div className="meta-key" style={{marginBottom:4}}>SHIPPER</div><div style={{whiteSpace:"pre-wrap",fontSize:11}}>{invoice.shipper||"窶・}</div></div>
                <div><div className="meta-key" style={{marginBottom:4}}>CONSIGNEE</div><div style={{whiteSpace:"pre-wrap",fontSize:11}}>{invoice.consignee||"窶・}</div></div>
              </div>
              <table>
              ...
              <table>
                ...
                <tbody>
                  {packingItems.map((p:any,i:number)=>{
                    const dim=[p.dimL,p.dimW,p.dimH].filter(Boolean).join("ﾃ・)||p.dimensions||"窶・;
                    if(p.mixed&&p.mixedItems?.length>0){
                      return p.mixedItems.map((mi:any,j:number)=>(
                        <tr key={`${i}-${j}`}>
                          <td>{j===0?p.cartonNo:""}</td>
                          <td style={{paddingLeft:j===0?6:20}}>{j>0?"笏・"}{mi.productName}</td>
      ...
      <div style={{display:"flex",justifyContent:"space-between"}}>
        <button className="btn btn-secondary" onClick={onBack}>竊・蜀・ｮｹ遒ｺ隱阪↓謌ｻ繧・/button>
        <button className="btn btn-primary" onClick={()=>{
          const email=invoice._consigneeEmail||"";
          const subject=`Invoice ${invoice.invoiceNo||""}`;
          const body=`縺贋ｸ冶ｩｱ縺ｫ縺ｪ縺｣縺ｦ縺翫ｊ縺ｾ縺吶・nInvoice ${invoice.invoiceNo||""}繧帝∽ｻ倥＞縺溘＠縺ｾ縺吶Ａ;
          window.location.href=`mailto:${email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
        }}>透 繝｡繝ｼ繝ｫ騾∽ｻ倥∈ 竊・/button>
      </div>
    </div>
  );
}
...
function HistoryPage({onLoad}:any) {
  ...
  const statusLabel:any={completed:"蜃ｺ闕ｷ螳御ｺ・,shipped:"霈ｸ騾∽ｸｭ",in_progress:"菴懈･ｭ荳ｭ",draft:"荳区嶌縺・};
  return (
    <div className="fade-in">
      <div className="grid-4" style={{marginBottom:20}}>
        {[{label:"邱乗｡井ｻｶ謨ｰ",value:SAMPLE_HISTORY.length,sub:"蜈ｨ譛滄俣"},{label:"莉頑怦蜃ｺ闕ｷ",value:2,sub:"莉ｶ"},{label:"菴懈･ｭ荳ｭ",value:1,sub:"莉ｶ"},{label:"荳区嶌縺・,value:1,sub:"莉ｶ"}]
          .map((s,i)=>(
      ...
      <div className="card">
        <div className="card-header">
          <div className="card-title">答 驕主悉譯井ｻｶ荳隕ｧ</div>
        ...
        <div style={{marginBottom:16}}>
          <input className="input" placeholder="剥 鬘ｧ螳｢蜷阪・Invoice No繝ｻ陬ｽ蜩∝錐繝ｻ蝗ｽ縺ｧ讀懃ｴ｢..." value={search} onChange={(e:any)=>setSearch(e.target.value)}/>
        </div>
        {filtered.map(h=>(
          <div key={h.id} className="history-item" onClick={()=>onLoad(h)}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
              <strong style={{fontSize:14}}>{h.id}</strong>
              <span className={`status-badge status-${h.status}`}>笳・{statusLabel[h.status]}</span>
            </div>
            <div style={{fontSize:13,color:"var(--text-muted)",marginBottom:6}}>{h.customer} 窶・{h.product}</div>
        ...
        {filtered.length===0&&<div className="empty-state"><div className="empty-icon">剥</div><div className="empty-text">隧ｲ蠖薙☆繧区｡井ｻｶ縺瑚ｦ九▽縺九ｊ縺ｾ縺帙ｓ</div></div>}
      </div>
    </div>
  );
}
...
function CustomerMasterPage({customers,setCustomers}:any) {
  ...
  return (
    <div className="fade-in">
      <div className="card">
        <div className="card-header">
          <div><div className="card-title">召 蜿門ｼ募・逋ｻ骭ｲ</div><div className="card-subtitle">逋ｻ骭ｲ縺励◆蜿門ｼ募・縺ｯInvoice菴懈・譎ゅ↓閾ｪ蜍募・蜉帙〒縺阪∪縺・/div></div>
          <button className="btn btn-primary btn-sm" onClick={()=>setShowForm(v=>!v)}>+ 蜿門ｼ募・霑ｽ蜉</button>
        </div>
        {showForm&&(
          <div style={{background:"#F7F7F5",borderRadius:"var(--radius-lg)",padding:16,marginBottom:16}}>
            <div className="grid-2" style={{marginBottom:12}}>
              <div className="field"><label className="label"><span className="required-dot">*</span>莨夂､ｾ蜷・/label>
              ...
              <div className="field"><label className="label">諡・ｽ楢・錐</label>
                <input className="input" value={form.contact} placeholder="逕ｰ荳ｭ 螟ｪ驛・ onChange={(e:any)=>setForm(v=>({...v,contact:e.target.value}))}/></div>
            </div>
            <div className="grid-2" style={{marginBottom:12}}>
              <div className="field"><label className="label">繝｡繝ｼ繝ｫ繧｢繝峨Ξ繧ｹ・域嶌鬘樣∽ｻ伜・・・/label>
            ...
            <div className="field" style={{marginBottom:12}}>
              <label className="label">Notify Party</label>
              <textarea className="input" rows={2} value={form.notifyParty} placeholder="騾夂衍蜈茨ｼ・/C逋ｺ陦梧凾縺ｪ縺ｩ・・ onChange={(e:any)=>setForm(v=>({...v,notifyParty:e.target.value}))}/>
            </div>
            <div className="grid-2" style={{marginBottom:12}}>
              <div className="field"><label className="label">繝・ヵ繧ｩ繝ｫ繝磯夊ｲｨ</label>
              ...
              <div className="field"><label className="label">繝・ヵ繧ｩ繝ｫ繝・ncoterms</label>
            ...
            <div style={{display:"flex",gap:8}}>
              <button className="btn btn-primary btn-sm" onClick={save}>菫晏ｭ・/button>
              <button className="btn btn-secondary btn-sm" onClick={()=>setShowForm(false)}>繧ｭ繝｣繝ｳ繧ｻ繝ｫ</button>
            </div>
          </div>
        )}
        {customers.length===0?(
          <div className="empty-state"><div className="empty-icon">召</div><div className="empty-text">蜿門ｼ募・繧堤匳骭ｲ縺励※縺上□縺輔＞</div></div>
        ):(
          customers.map((c:any)=>(
            <div key={c.id} className="history-item">
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <strong style={{fontSize:14}}>{c.name}</strong>
                <button className="btn btn-danger btn-xs" onClick={()=>setCustomers((v:any[])=>v.filter((x:any)=>x.id!==c.id))}>蜑企勁</button>
              </div>
              <div className="history-meta" style={{marginTop:6}}>
                ...
                {c.email&&<span className="tag tag-purple">笨・{c.email}</span>}
              </div>
...
function ProductMasterPage({products,setProducts}:any) {
  ...
  return (
    <div className="fade-in">
      <div className="card">
        <div className="card-header">
          <div><div className="card-title">翌・・陬ｽ蜩√・繧ｹ繧ｿ</div><div className="card-subtitle">逋ｻ骭ｲ縺励◆陬ｽ蜩√・Invoice菴懈・譎ゅ↓閾ｪ蜍戊｣懷ｮ後〒縺阪∪縺・/div></div>
          <button className="btn btn-primary btn-sm" onClick={()=>setShowForm(v=>!v)}>+ 陬ｽ蜩∬ｿｽ蜉</button>
        </div>
        {showForm&&(
          <div style={{background:"#F7F7F5",borderRadius:"var(--radius-lg)",padding:16,marginBottom:16}}>
            <div className="grid-2" style={{marginBottom:12}}>
              <div className="field"><label className="label"><span className="required-dot">*</span>陬ｽ蜩∝錐</label>
                <input className="input" value={form.name} placeholder="Product Name" onChange={(e:any)=>setForm(v=>({...v,name:e.target.value}))}/></div>
              <div className="field"><label className="label">HS繧ｳ繝ｼ繝・/label>
            ...
            <div className="grid-4" style={{marginBottom:12}}>
              ...
              <div className="field"><label className="label">騾夊ｲｨ</label>
                <select className="input" value={form.currency} onChange={(e:any)=>setForm(v=>({...v,currency:e.target.value}))}>
                  {CURRENCIES.map((c:string)=><option key={c}>{c}</option>)}</select></div>
              <div className="field"><label className="label">驥埼㍼(kg/蛟・</label>
        ...
        {products.length===0?(
          <div className="empty-state"><div className="empty-icon">翌・・/div><div className="empty-text">陬ｽ蜩√ｒ逋ｻ骭ｲ縺励※縺上□縺輔＞</div></div>
        ):(
          products.map((p:any)=>(
            <div key={p.id} className="history-item">
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <strong style={{fontSize:14}}>{p.name}</strong>
                <button className="btn btn-danger btn-xs" onClick={()=>setProducts((v:any[])=>v.filter((x:any)=>x.id!==p.id))}>蜑企勁</button>
              </div>
              <div className="history-meta" style={{marginTop:6}}>
                ...
                {p.weight&&<span className="tag tag-amber">{p.weight}kg/蛟・/span>}
              </div>
            </div>
          ))
        )}
...
function OrgSettingsPage({orgSettings,setOrgSettings}:any) {
  ...
  return (
    <div className="fade-in">
      <div className="card">
        <div className="card-header"><div><div className="card-title">笞呻ｸ・邨・ｹ碑ｨｭ螳・/div><div className="card-subtitle">PDF譖ｸ鬘槭↓蜿肴丐縺輔ｌ繧玖・遉ｾ諠・ｱ繧定ｨｭ螳壹＠縺ｾ縺・/div></div></div>

        {/* 繝ｭ繧ｴ */}
        <div style={{marginBottom:20}}>
          <div className="label" style={{marginBottom:8}}>閾ｪ遉ｾ繝ｭ繧ｴ</div>
          ...
          <button className="btn btn-secondary btn-sm" onClick={()=>fileRef.current?.click()}>名・・繝ｭ繧ｴ繧偵い繝・・繝ｭ繝ｼ繝・/button>
          {orgSettings.logoUrl&&<button className="btn btn-danger btn-sm" style={{marginLeft:8}} onClick={()=>setOrgSettings((v:any)=>({...v,logoUrl:""}))}>蜑企勁</button>}
        </div>
        ...
        <div className="grid-2" style={{marginBottom:16}}>
          <div className="field"><label className="label">莨夂､ｾ蜷・/label>
            <input className="input" value={orgSettings.companyName||""} placeholder="譬ｪ蠑丈ｼ夂､ｾ縲・・ onChange={(e:any)=>setOrgSettings((v:any)=>({...v,companyName:e.target.value}))}/></div>
          <div className="field"><label className="label">諡・ｽ楢・錐・育ｽｲ蜷搾ｼ・/label>
            <input className="input" value={orgSettings.signatureName||""} placeholder="逕ｰ荳ｭ 螟ｪ驛・ onChange={(e:any)=>setOrgSettings((v:any)=>({...v,signatureName:e.target.value}))}/></div>
        </div>
        <div className="grid-2" style={{marginBottom:16}}>
        ...
        <div className="field" style={{marginBottom:16}}>
          <label className="label">閾ｪ遉ｾ菴乗園</label>
          <textarea className="input" rows={3} value={orgSettings.address||""} placeholder={"縲・00-0000\n譚ｱ莠ｬ驛ｽ縲・・玄縲・・-1-1\nJapan"} onChange={(e:any)=>setOrgSettings((v:any)=>({...v,address:e.target.value}))}/>
        </div>
        ...
        {/* 驫陦梧・邏ｰ */}
        <div className="field" style={{marginBottom:16}}>
          <label className="label">謾ｯ謇募・驫陦梧・邏ｰ・・DF縺ｫ蜊ｰ蛻ｷ縺輔ｌ縺ｾ縺呻ｼ・/label>
          <textarea className="input" rows={5} value={orgSettings.bankDetails||""} placeholder={"Bank Name: 縲・・橿陦圭nBranch: 縲・・髪蠎予nAccount Type: 譎ｮ騾喀nAccount No: 1234567\nAccount Name: 譬ｪ蠑丈ｼ夂､ｾ縲・ⅨnSWIFT Code: XXXXXXXX"} onChange={(e:any)=>setOrgSettings((v:any)=>({...v,bankDetails:e.target.value}))}/>
        </div>

        <div style={{display:"flex",justifyContent:"flex-end"}}>
          <button className="btn btn-primary" onClick={()=>alert("險ｭ螳壹ｒ菫晏ｭ倥＠縺ｾ縺励◆・・)}>沈 險ｭ螳壹ｒ菫晏ｭ・/button>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const navItems=[
    {id:"new",label:"譁ｰ隕丈ｽ懈・",icon:"笨擾ｸ・},
    {id:"history",label:"驕主悉譯井ｻｶ",icon:"答",badge:SAMPLE_HISTORY.filter(h=>h.status==="in_progress").length||null},
    {id:"customers",label:"蜿門ｼ募・逋ｻ骭ｲ",icon:"召"},
    {id:"products",label:"陬ｽ蜩√・繧ｹ繧ｿ",icon:"翌・・},
    {id:"settings",label:"邨・ｹ碑ｨｭ螳・,icon:"笞呻ｸ・},
  ];
  const topbarTitle:any={new:"譁ｰ隕乗嶌鬘樔ｽ懈・",history:"驕主悉譯井ｻｶ讀懃ｴ｢",customers:"蜿門ｼ募・逋ｻ骭ｲ",products:"陬ｽ蜩√・繧ｹ繧ｿ",settings:"邨・ｹ碑ｨｭ螳・};
      ...
      <div className="app">
        <aside className="sidebar">
          <div className="sidebar-logo">
            ...
            <div className="logo-sub">{orgSettings.companyName||"雋ｿ譏捺嶌鬘樒ｮ｡逅・す繧ｹ繝・Β"}</div>
          </div>
          <nav className="sidebar-nav">
            <div className="nav-section-label">繝｡繝九Η繝ｼ</div>
            {navItems.map(n=>(
            ...
            <div className="nav-section-label" style={{marginTop:16}}>譛霑代・譯井ｻｶ</div>
            {SAMPLE_HISTORY.slice(0,3).map(h=>(
              <button key={h.id} className="nav-item" onClick={()=>loadHistory(h)}>
                <span className="nav-icon">塘</span>
            ...
            {validationResult.errors.length>0&&(
              <button style={{margin:"16px 8px 0",padding:"8px 12px",background:"var(--red-light)",borderRadius:"var(--radius)",fontSize:12,color:"var(--red)",border:"none",cursor:"pointer",width:"calc(100% - 16px)",textAlign:"left"}}
                ...
                笞・・{validationResult.errors.length}莉ｶ縺ｮ繧ｨ繝ｩ繝ｼ 竊・遒ｺ隱咲判髱｢縺ｸ
              </button>
            )}
          </nav>
        </aside>
        ...
        <main className="main">
          <div className="topbar">
            ...
            <div className="topbar-actions">
              {page==="new"&&<button className="btn btn-secondary btn-sm" onClick={reset}>売 繝ｪ繧ｻ繝・ヨ</button>}
              {page==="new"&&<button className="btn btn-primary btn-sm" onClick={()=>setPage("history")}>答 驕主悉譯井ｻｶ繧貞盾辣ｧ</button>}
            </div>
          </div>
