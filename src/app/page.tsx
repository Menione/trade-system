...
前回、私がコードを2つに分けてお渡しした際、**前半と後半で同じ機能（Appコンポーネントなど）が重複して含まれてしまっており、それが原因でコードがバグを起こしてページ移動できなくなっていました。**

今回は、絶対に重複が起こらないよう、ファイルをきっちり半分（前半と後半）に分けて出力します。

大変お手数ですが、以下の手順で**まっさらな状態から貼り付け直し**をお願いいたします。

### 確実な修正手順

1. 現在の `page.tsx` の中身を **すべて全選択して削除（空っぽに）** してください。
2. 以下の **【前半】** の右上にある「コピー」ボタンを押して、`page.tsx` に貼り付けます。
3. 続けて、以下の **【後半】** の右上にある「コピー」ボタンを押して、**【前半】のコードの一番下の行に続けて**貼り付け、保存してください。

---

### 【前半】（空の page.tsx の一番上に貼り付け）
